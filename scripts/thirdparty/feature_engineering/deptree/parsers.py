...
from scripts.thirdparty.module.spacy import Spacy
...
class CorenlpParser(Parser):
    ...
    def parse(sent):
        ...
        """
        c = re.sub(r'\s{2,}', ' ', sent.content)
        ann = Corenlp.annotate(c, annotators=[Corenlp.DEPPARSE])
        edges = []

        if len(ann.sentence) > 1:
            raise ValueError('Sentence can be split.')

        tree = ann.sentence[0].enhancedDependencies
        for edge in tree.edge:
            fro = sent.tokens[edge.source - 1]
            to = sent.tokens[edge.target - 1]
            edge = (edge.dep, fro, to)
            edges.append(edge)
